...
static void decode_mb(MpegEncContext *s, int ref)
{
    ...
    if (CONFIG_H264_DECODER && s->codec_id == AV_CODEC_ID_H264) {
        ...
        if (ref >= h->ref_count[0])
            ref = 0;
        fill_rectangle(&s->current_picture.ref_index[0][4 * h->mb_xy],
                       2, 2, 2, ref, 1);
                     ...
                     int h, int stride, int is_luma)
{
    ...
    for (b_y = 0; b_y < h; b_y++) {
        for (b_x = 0; b_x < w; b_x++) {
            ...
            if (IS_INTER(s->current_picture.mb_type[mb_index]))
                continue; // inter
                ...
                for (mb_x = 0; mb_x < s->mb_width; mb_x++) {
                    ...
                    if (mb_x > 0 && fixed[mb_xy - 1]) {
                        mv_predictor[pred_count][0] =
                            s->current_picture.motion_val[0][mot_index - mot_step][0];
                        mv_predictor[pred_count][1] =
                            s->current_picture.motion_val[0][mot_index - mot_step][1];
                        ref[pred_count] =
                    ...
                    if (mb_y > 0 && fixed[mb_xy - mb_stride]) {
                        mv_predictor[pred_count][0] =
                            s->current_picture.motion_val[0][mot_index - mot_stride * mot_step][0];
                        mv_predictor[pred_count][1] =
                            s->current_picture.motion_val[0][mot_index - mot_stride * mot_step][1];
                        ref[pred_count] =
                            s->current_picture.ref_index[0][4 * (mb_xy - s->mb_stride)];
                        pred_count++;
                    }
...
skip_last_mv:
                    ...
                    for (j = 0; j < pred_count; j++) {
                        ...
                        s->current_picture.motion_val[0][mot_index][0] =
                            s->mv[0][0][0] = mv_predictor[j][0];
                        s->current_picture.motion_val[0][mot_index][1] =
...
void ff_er_frame_end(MpegEncContext *s)
{
    ...
    if (s->current_picture.motion_val[0] == NULL) {
        ...
        for (i = 0; i < 2; i++) {
            pic->ref_index_buf[i]  = av_buffer_allocz(s->mb_stride * s->mb_height * 4 * sizeof(uint8_t));
            pic->motion_val_buf[i] = av_buffer_allocz((size + 4) * 2 * sizeof(uint16_t));
            if (!pic->ref_index_buf[i] || !pic->motion_val_buf[i])
                return;
            pic->ref_index[i]  = pic->ref_index_buf[i]->data;
            pic->motion_val[i] = (int16_t (*)[2])pic->motion_val_buf[i] + 4;
        }
        pic->f.motion_subsample_log2 = 3;
        ff_mpeg_unref_picture(s, &s->current_picture);
        if (ff_mpeg_ref_picture(s, &s->current_picture, s->current_picture_ptr) < 0)
            return;
    }
