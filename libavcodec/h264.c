...
static int decode_slice_header(H264Context *h, H264Context *h0)
{
    ...
    if ((h->avctx->flags2 & CODEC_FLAG2_FAST) &&
        ...
        h->me.qpel_put = h->dsp.put_2tap_qpel_pixels_tab;
        h->me.qpel_avg = h->dsp.avg_2tap_qpel_pixels_tab;
    } else {
        h->me.qpel_put = h->dsp.put_h264_qpel_pixels_tab;
        h->me.qpel_avg = h->dsp.avg_h264_qpel_pixels_tab;
    }
...
again:
            ...
            case NAL_SLICE:
                ...
                break;
            case NAL_DPA:
                if (h->avctx->flags2 & CODEC_FLAG2_CHUNKS) {
                    av_log(h->avctx, AV_LOG_ERROR,
